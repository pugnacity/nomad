...
- `limit_query` `(int: 20)` - Specifies the maximum number of Nomad objects to
  search through in the Nomad server before truncating results. Setting this parameter
  to high value may degrade Nomad server performance.

- `limit_results` `(int: 100)` - Specifies the maximum number of matching results
  to accumulate in the API response before truncating results. Setting this parameter
  to a high value may cause excessively large API response sizes.
